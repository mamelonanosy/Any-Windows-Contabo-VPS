...
part1_size_mb=$((15 * 1024)) # 15 GB in MB
part2_size_mb=$((17 * 1024)) # 17 GB in MB
...
if [[ "$download_choice" == "Y" || "$download_choice" == "y" ]]; then
    ...
    if [ -z "$windows_url" ]; then
        windows_url="https://rb.gy/qpd6bs"  # Replace with the actual default URL
    fi
...
if [[ "$download_choice" == "Y" || "$download_choice" == "y" ]]; then
    ...
    if [ -z "$virtio_url" ]; then
        virtio_url="https://shorturl.at/aiy13"  # Replace with the actual default URL
    fi
